/**************************************************************************
  * @file       : simple_byte_send_server.h
...
#ifndef SIMPLE_BYTE_SEND_SERVER_H__
#define SIMPLE_BYTE_SEND_SERVER_H__

#include <stdint.h>
#include <stdbool.h>
#include "access.h"

/** Simple ByteSend Server model ID. */
#define SIMPLE_BYTE_SEND_SERVER_MODLE_ID (0x0004)

/** Forward declaration. */
typedef struct simple_byte_send_server simple_byte_send_server_t;

/**
 * Get callback type.
 * @param[in] p_self Pointer to the Simple ByteSend Server context structure.
 * @returns @c true if the state is On, @c false otherwise.
 */
typedef uint8_t (*simple_byte_send_get_cb_t)(const simple_byte_send_server_t * p_self);
 ...
 */
typedef uint8_t (*simple_byte_send_set_cb_t)(const simple_byte_send_server_t * p_self, uint8_t msg);

/** Simple ByteSend Server state structure. */
struct simple_byte_send_server
{
	/** Model handle assigned to the server. */
    access_model_handle_t model_handle;
    /** Get callback. */
    simple_byte_send_get_cb_t get_cb;
    /** Set callback. */
    simple_byte_send_set_cb_t set_cb;
};
 ...
 *
 * @param[in] p_server      Simple ByteSend Server structure pointer.
...
#endif //SIMPLE_BYTE_SEND_SERVER_H__